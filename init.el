...
values."
  (setq-default
   ...
   '(javascript
     ...
     syntax-checking
     ;; version-control
     ;;mac osx(在emacs下依旧能使用mac命令，如复制粘贴等，非mac os系统可以忽略)
     osx
...
`dotspacemacs/user-config' first."
  (setq-default
    ;; master 分支 configuration-layer--elpa-archives
    ;; develop 分支configuration-layer-elpa-archives
...
;; 平时的一些想法和笔记
(setq org-agenda-file-note "~/Desktop/lxl/Note/suiji/lxl-agenda/notes.org")
;; 存放临时任务
(setq org-agenda-file-task "~/Desktop/lxl/Note/suiji/lxl-agenda/task.org")
;; 用来存放每日，每周， 每月固定的任务
(setq org-agenda-file-calendar "~/Desktop/lxl/Note/suiji/lxl-agenda/calendar.org")
;; 用来存放已经完成的任务
(setq org-agenda-file-finished "~/Desktop/lxl/Note/suiji/lxl-agenda/finished.org")
;; 用来存放已经取消的任务
(setq org-agenda-file-canceled "~/Desktop/lxl/Note/suiji/lxl-agenda/canceled.org")
...
(setq org-capture-templates
      '(
        ("t" "Todo" entry (file+headline org-agenda-file-task "工作相关")
         "* TODO [#B] %?\n %<%Y-%m-%d %H:%M> %i\n" :empty-lines 1)
        ("l" "Tolearn" entry (file+headline org-agenda-file-task "学习相关")
         "* TODO [#B] %?\n  %i\n" :empty-lines 1)
        ("h" "Toplay" entry (file+headline org-agenda-file-task "好玩的")
         "* TODO [#C] %?\n  %i\n"  :empty-lines 1)
